...
# Create IAM role
try {
    $roleOutput = aws iam create-role --role-name $IAM_ROLE_NAME --assume-role-policy-document file://trust-policy.json 2>$null
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ IAM role created successfully" -ForegroundColor Green
    } else {
        Write-Host "ℹ️ IAM role already exists, continuing..." -ForegroundColor Yellow
    }
} catch {
    Write-Host "ℹ️ IAM role creation skipped (may already exist)" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "⚠️ Failed to attach IAM policy. Please check manually." -ForegroundColor Yellow
}
...
Create-LambdaPackage -FunctionFile "lambda_s3_document_fetcher.py" -ZipName "s3-fetcher.zip"
Create-LambdaPackage -FunctionFile "lambda_bedrock_claude_analyzer.py" -ZipName "bedrock-analyzer.zip"
Create-LambdaPackage -FunctionFile "lambda_prism_database_integration.py" -ZipName "prism-integration.zip"
...
try {
    aws lambda create-function `
        ...
        --timeout 300 `
        --memory-size 1024 `
        --description "Fetches regulatory documents from S3 bucket"
...
try {
    aws lambda create-function `
        ...
        --timeout 300 `
        --memory-size 1024 `
        --description "Analyzes documents using Amazon Bedrock Claude"
...
try {
    aws lambda create-function `
        ...
        --timeout 300 `
        --memory-size 1024 `
        --description "Integrates with PRISM database for document matching"
...
try {
    $apiOutput = aws apigateway create-rest-api --name $API_NAME --description "PRISM Regulatory Intelligence API"
    $API_ID = ($apiOutput | ConvertFrom-Json).id
    Write-Host "✅ API Gateway created: $API_ID" -ForegroundColor Green
} catch {
    Write-Host "⚠️ API Gateway creation failed. Please create manually." -ForegroundColor Yellow
}
